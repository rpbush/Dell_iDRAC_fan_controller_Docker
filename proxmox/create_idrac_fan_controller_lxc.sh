...
# Admin user workaround for iDRAC9 root user restrictions
...
create_status_script() {
  echo "Downloading status monitoring script from GitHub"
  
  # Download the status script directly from GitHub
  local status_script_url="https://raw.githubusercontent.com/rpbush/Dell_iDRAC_fan_controller_Docker/master/proxmox/idrac-status.sh"
  
  echo "DEBUG: Downloading status script from $status_script_url"
  exec_in_container "curl -fsSL '$status_script_url' -o /usr/local/bin/idrac-status"
